class BlankForm < ActionView::Helpers::FormBuilder  
  CSS = {
    :label => '',
    :hint => 'hint',
    :hint_ptr => 'hint-pointer',
    :error => 'help-inline',            # style for error text
    :field_error => 'error'    # style for error field
  }
  ...
  def self.create_tagged_field(method_name)
    define_method(method_name) do |attribute, *args|
      ...
      if args.last.is_a?(Hash)
        ...
        main_class = args.last[:main_class]
        sub_class = args.last[:sub_class]
      end
      ...
      if @object and @object.errors.any?
        ...
        if error.present?
          ...
          error_tag = @template.content_tag(:span, error.first, {:class => style})
          args.last[:class] = "#{args.last[:class]} error"
          error_class = "error"
        end
      end
      ...
      if required(attribute)
        label_tag = label(attribute, "#{label_txt or attribute.to_s.titleize} *") 
        args.last[:required] = "required"
      else
        label_tag = label(attribute, "#{label_txt or attribute.to_s.titleize}") 
      end

      base_tag = @template.content_tag(:div, super(attribute, *args) + error_tag, :class=> "input")
      ...
      all_tags = label_tag + base_tag + spinner_tag + hint_tag 
      # TODO: Remove the css_class special fix -- use :pre instead
      if css_class =~ /input_small|input_tiny/ or pre
        all_tags
      else
        # Wrap all the form fields inside a <div> tag and add a lable to them
        main_tag = @template.content_tag(:div, all_tags, :class => "clearfix #{error_class}") 
        css_class = main_class if main_class
        @template.content_tag(:div, main_tag, :class => "#{css_class} #{sub_class}") 
      end
    end
  end
